...
class Evaluator:
    ...
    def __init__(self,):
        self.exp_dir = cfg.result_dir + '/metrics'
        os.system('mkdir -p ' + cfg.result_dir)
        os.system('mkdir -p ' + self.exp_dir)

    def evaluate(self, output, sample):
        metrics = {}

        pred_trajectory, __, pred_visibility, __ = output
        ...
        if "tapvid" in cfg.eval.dataset_name:
            ...
            out_metrics = compute_tapvid_metrics(
                ...
                pred_occluded,
                pred_tracks,
                query_mode="strided" if "strided" in cfg.eval.dataset_name else "first",
            )
