...
from lib.networks.cotracker.network_utils import *
from lib.networks.encoding.cotracker.embeddings import *
...
class Network(nn.Module):
    def __init__(self,):
        ...
        net_cfg = cfg.network
        self.S = net_cfg.S
        self.stride = net_cfg.stride
        self.add_space_attn = net_cfg.add_space_attn
        self.latent_dim = net_cfg.latent_dim

        # UpdateFormer
        self.num_heads = net_cfg.update_former.num_heads
        self.hidden_size = net_cfg.update_former.hidden_size
        self.space_depth = net_cfg.update_former.space_depth
        self.time_depth = net_cfg.update_former.time_depth

        # CorrBlock
        self.corr_levels = net_cfg.corr_block.corr_levels
        self.corr_radius = net_cfg.corr_block.corr_radius

        self.fnet = BasicEncoder(
            output_dim=self.latent_dim, norm_fn="instance", dropout=0, stride=self.stride
        )
